...
var argv = yargs
    ...
    .version("1.0")
    .usage('Usage:\n  apigee-edge-auth.js -u username@google.com -p SuperSecret')
    .argv;
...
(async function main() {
    ...
    try {
        try {
            await page.goto('https://login.apigee.com/');
        } catch(ex) {
            throw new ScriptError(`Could not load Apigee page. ${ex.message}`);
        }

        //Fill in username
        let emailFieldSelector = 'input[name="username"]';
        ...
        debug_log('Clicking Sign in  ...');
        await clickButtonWithText(page, ['Sign in']);

        try {
            await waitForField(page, 'csrf', 5000, 'Apigee page did not load');
        } catch(ex) {
            if (await pageHasText(page, ["Unable to", "Couldn't sign", "Sign in", "Wrong password"])) {
